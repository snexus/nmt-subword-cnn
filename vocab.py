...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        return [[[self.start_of_word] + [self.char2id.get(x, self.char_unk) for x in w] + [self.end_of_word] for w in s] for s in
                sents]
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        ###     previous parts
        wordchar_ids = self.words2charindices(sents)
        sents_p = pad_sents_char(wordchar_ids,  self.char2id['<pad>'])
        sents_var = torch.tensor(sents_p, dtype=torch.long, device=device)
        return sents_var.permute(1,0,2) # Swap batch and max_sentence dimensions
