...
class NMT(nn.Module):
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        #source_padded = self.vocab.src.to_input_tensor(source, device=self.device)   # Tensor: (src_len, b)
        target_padded = self.vocab.tgt.to_input_tensor(target, device='cpu')
        source_padded_chars = self.vocab.src.to_input_tensor_char(source, device='cpu')
        target_padded_chars = self.vocab.tgt.to_input_tensor_char(target, device='cpu')
